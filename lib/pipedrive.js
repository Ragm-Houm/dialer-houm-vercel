...
const IS_JWT_TOKEN = !!API_TOKEN && API_TOKEN.split('.').length === 3;

// Headers de autenticación para Pipedrive
  ...
  try {
    ...
    if (response.data && response.data.success) {
      return {
        ...
        link: `https://${DOMAIN}.pipedrive.com/deal/${dealId}`
      };
    }
...
async function listDealsByPipelineStage(pipelineId, stageId, limit = 100, options = {}) {
  try {
    ...
    while (deals.length < limit) {
      const url = `${BASE_URL}/deals?status=open&pipeline_id=${pipelineId}&stage_id=${stageId}&start=${start}&limit=${pageSize}&sort=add_time%20ASC&api_token=${encodeURIComponent(API_TOKEN)}`;
...
async function getDealContext(dealId) {
  try {
    const deal = await getDealById(dealId);
    const pipelineId = deal && deal.pipeline_id
      ? (typeof deal.pipeline_id === 'number' ? deal.pipeline_id : deal.pipeline_id.id)
      : null;
...
async function listLostReasons() {
  try {
    ...
    if (response.data) {
      ...
      if (raw.length > 0) {
        return raw.map((item) => ({
          id: item.id || item.value || item.key || item.label,
          label: item.label || item.name || item.value || item.key || 'Sin nombre',
          active_flag: item.active_flag
        }));
      }
    }

    // Fallback: algunos tenants exponen otro endpoint
    try {
      const altUrl = `${BASE_URL}/deals/lostReasons`;
      const altResponse = await axios.get(altUrl, getAxiosConfig());
      if (altResponse.data) {
        const altRaw = Array.isArray(altResponse.data)
          ? altResponse.data
          : (altResponse.data.data || []);
        if (altRaw.length > 0) {
          return altRaw.map((item) => ({
            id: item.id || item.value || item.key || item.label,
            label: item.label || item.name || item.value || item.key || 'Sin nombre',
            active_flag: item.active_flag
          }));
      ...
      console.warn('Fallback lostReasons endpoint failed:', altError.message);
    }

    // Fallback: algunos tenants exponen motivos en dealFields
    const fieldsUrl = `${BASE_URL}/dealFields`;
    const fieldsResponse = await axios.get(fieldsUrl, getAxiosConfig());
    if (fieldsResponse.data && fieldsResponse.data.success) {
      const fields = fieldsResponse.data.data || [];
      const lostField =
        fields.find((field) => field.key === 'lost_reason') ||
        fields.find((field) => field.key === 'lost_reason_id') ||
        fields.find((field) => {
          const name = String(field.name || '').toLowerCase();
          return (
            name.includes('motivo de pérdida') ||
            name.includes('motivo de perdida') ||
            name.includes('lost reason') ||
            name.includes('reason')
          );
        });
      if (lostField && Array.isArray(lostField.options)) {
        return lostField.options.map((opt) => ({
          id: opt.id || opt.value || opt.key || opt.label,
          label: opt.label || opt.name || opt.value || opt.key || 'Sin nombre',
...
async function listStages(pipelineId) {
  try {
    assertPipedriveConfig();
    const normalizedPipelineId = typeof pipelineId === 'object' && pipelineId !== null
      ? pipelineId.id
      : pipelineId;
    const url = `${BASE_URL}/stages?pipeline_id=${normalizedPipelineId}`;
    const response = await axios.get(url, {
      ...getAxiosConfig(),
      validateStatus: () => true
    });

    console.log(`[listStages] pipeline=${pipelineId} status=${response.status} success=${response.data?.success} count=${(response.data?.data || []).length}`);

    if (response.status === 401 || response.status === 403) {
      console.error(`[listStages] Auth error ${response.status}:`, JSON.stringify(response.data).slice(0, 300));
      return [];
    }
    ...
    // Pipedrive returns success:true with data:null when no stages found
    if (response.data && response.data.success === false) {
      console.error('[listStages] Pipedrive error:', JSON.stringify(response.data).slice(0, 300));
    }

    return [];
  } catch (error) {
    console.error('Error obteniendo etapas en Pipedrive:', error.message, error.response?.status, JSON.stringify(error.response?.data || '').slice(0, 300));
    return [];
  }
}
