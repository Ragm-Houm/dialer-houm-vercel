...
// Limpiar variables de entorno (remover comillas y espacios)
...
const apiKey = process.env.TWILIO_API_KEY?.trim().replace(/^["']|["']$/g, '');
const apiSecret = process.env.TWILIO_API_SECRET?.trim().replace(/^["']|["']$/g, '');
const twimlAppSid = process.env.TWILIO_TWIML_APP_SID?.trim().replace(/^["']|["']$/g, '');
