...
// Fallback en memoria por si document.cookie no refleja el token
let _csrfTokenMemory = null;

/**
 * Lee el token CSRF de las cookies o memoria
 */
export function getCsrfToken() {
  if (typeof document === 'undefined') return _csrfTokenMemory;

  const cookies = document.cookie.split(';');
  for (const cookie of cookies) {
    const [name, value] = cookie.trim().split('=');
    if (name === CSRF_COOKIE_NAME && value) {
      _csrfTokenMemory = value;
      return value;
    }
  }
  // Fallback a memoria si cookie no se lee
 ...
 */
export async function initCsrf(forceRefresh = false) {
  try {
    ...
    // Guardar token en memoria como fallback
    try {
      const data = await res.json();
      if (data.token) {
        _csrfTokenMemory = data.token;
      }
    } catch (_) {}
...
export async function csrfFetch(url, options = {}) {
  ...
  // Si no hay CSRF token, forzar regeneración antes de la request
  if (!csrfToken) {
    await initCsrf(true);
  ...
  if (res.status === 403) {
    try {
      ...
      if (body?.code === 'CSRF_VALIDATION_FAILED') {
        // Forzar regeneración del token CSRF
        await initCsrf(true);
