...
export default function Home() {
  ...
  const loadCampaigns = async (nextCountry = pais) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/campaigns?country=${nextCountry}&view=dialer&email=${encodeURIComponent(email)}`
      );
  ...
  const POSITIVE_BUCKETS = new Set(['contacto_efectivo', 'conversion', 'interesado', 'agendado', 'publicada', 'reservada', 'arrendada']);
  const NEUTRAL_BUCKETS = new Set(['no_contacto', 'seguimiento', 'no_contesta', 'futuro']);
  ...
  const DEFAULT_OUTCOMES = [
    ...
    { key: 'le_parece_caro', label: 'Le parece caro', outcome_type: 'final', metric_bucket: 'descarte', category: 'negative', action_config: { assign_owner: true, allow_change_owner: true, change_stage: 'no', log_pipedrive: true, mark_lost: true, require_lost_reason: true, create_followup: false, mark_done: true, allow_retry: false, require_retry_time: false, require_future_delay: false } },
    { key: 'intentos_max', label: 'Intentos máximos', outcome_type: 'final', metric_bucket: 'descarte', category: 'negative', action_config: { assign_owner: true, allow_change_owner: true, change_stage: 'no', log_pipedrive: true, mark_lost: true, require_lost_reason: true, create_followup: false, mark_done: true, allow_retry: false, require_retry_time: false, require_future_delay: false } }
  ];
